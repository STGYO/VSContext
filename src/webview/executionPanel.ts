...
function renderExecutionHtml(webview: vscode.Webview, result: ExecutionTraceResult): string {
...
<body>
  ...
  <p class="summary">Nodes visited: ${result.nodes.length.toString()} | Max depth: ${result.maxDepth.toString()}</p>
  <div class="graph-shell">
    <svg id="trace-graph" aria-label="Execution trace graph"></svg>
  </div>
  ...
  <script nonce="${nonce}">
    ...
    function renderGraph() {
      ...
      if (!graphData.nodes || graphData.nodes.length === 0) {
        ...
        paragraph.textContent = 'No nodes available for this execution trace.';
