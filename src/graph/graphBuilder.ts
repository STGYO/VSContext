...
import {
  ...
  type WorkspaceFileRoleSummary,
} from "../utils/fileRoleClassifier";
import {
...
export type GraphEdgeType = Extract<
  KnowledgeRelationshipKind,
  "calls" | "implements" | "reads" | "writes"
>;

export type WorkspaceFileRelationshipType = Extract<
  KnowledgeRelationshipKind,
  "imports" | "covers" | "documents" | "related-to"
>;
...
export class WorkspaceGraphBuilder {
  ...
  private supplementaryFileRelationshipIndex = new Map<
    string,
    WorkspaceFileRelationship[]
  >();
  private persistTimer: NodeJS.Timeout | undefined;
  private persistQueue: Promise<void> = Promise.resolve();
  private lastIndexResult: WorkspaceIndexResult | undefined;

  /** SQLite database for persistent graph storage (Phase 9A). */
  private db: GraphDatabase | undefined;
  private dbUri: vscode.Uri | undefined;
  ...
  public getLastIndexResult(): WorkspaceIndexResult | undefined {
  ...
  public async hydrateFromCache(): Promise<boolean> {
    ...
    if (this.db) {
      ...
      if (dbSnapshot.nodes.size > 0) {
        ...
        const restoredSymbolCache = new Map<string, IndexedSymbol>();
        for (const [nodeId, jsonStr] of dbSnapshot.symbolCacheJson) {
          try {
            const sym = deserializeIndexedSymbol(
              JSON.parse(jsonStr) as SerializedIndexedSymbol,
            );
            if (sym) {
        ...
        for (const node of nodeMap.values()) {
          if (!restoredSymbolCache.has(node.id)) {
            restoredSymbolCache.set(
              node.id,
              this.toIndexedSymbolFromNode(node),
            );
          }
        }
        ...
        for (const rel of dbSnapshot.fileRelationships) {
          const entry: WorkspaceFileRelationship = {
            ...
            relationship: rel.relationship as WorkspaceFileRelationshipType,
          };
        ...
        const fileRoleSummary = dbSnapshot.fileRoleSummaryJson
          ? this.parseFileRoleSummary(
              JSON.parse(dbSnapshot.fileRoleSummaryJson) as unknown,
            )
          : undefined;
        ...
        const edgeCount = [...nodeMap.values()].reduce(
          (count, node) => count + this.relationshipCountForNode(node),
          0,
        );
        this.logger.info(
          `Hydrated workspace graph from SQLite with ${nodeMap.size} nodes and ${edgeCount} edges.`,
        );
        return true;
      }
    ...
    try {
      ...
      this.logger.warn(
        `Unable to read VSContext graph cache. Falling back to full rebuild. ${String(error)}`,
      );
      return false;
    }
    ...
    try {
      ...
      this.logger.warn(
        "VSContext graph cache is not valid JSON. Falling back to full rebuild.",
      );
      return false;
    }
    ...
    if (!snapshot) {
      this.logger.warn(
        "VSContext graph cache schema is invalid. Falling back to full rebuild.",
      );
      return false;
    }
    ...
    const restoredSymbolCache = deserializeIndexedSymbolMap(
      snapshot.symbolCache,
    );
    for (const node of nodeMap.values()) {
      if (!restoredSymbolCache.has(node.id)) {
    ...
    this.cachedGraph = {
      nodes: nodeMap,
      fileIndex: this.createFileIndex(nodeMap),
      fileRelationships: this.deserializeWorkspaceFileRelationships(
        snapshot.fileRelationships,
      ),
      builtAt: isBuiltAtValid ? builtAt : undefined,
      fileRoleSummary: snapshot.fileRoleSummary,
    };
    this.symbolCache = restoredSymbolCache;
    this.fileModifiedTimes = this.deserializeFileModifiedTimes(
      snapshot.fileModifiedTimes,
    );
    this.rebuildFileRelationshipIndex(nodeMap);
    this.initialIndexCompleted = true;
    this.dirty = false;
    ...
    this.logger.info(
      `Hydrated workspace graph from JSON cache with ${nodeMap.size} nodes and ${edgeCount} edges.`,
    );

    // If a DB is open, migrate the JSON data into SQLite and delete the file.
    if (this.db) {
      this.logger.info(
        "[VSContext] Migrating JSON graph cache to SQLite database.",
      );
    ...
    if (this.db) {
      ...
      db.transaction(() => {
        ...
        for (const symbol of symbols) {
          ...
          if (cached) {
            db.upsertSymbolCache(
              symbol.id,
              JSON.stringify(serializeIndexedSymbol(cached)),
            );
          }
        }
  ...
  private async doBuildWorkspaceGraph(): Promise<WorkspaceGraph> {
    ...
    try {
      ...
      await this.populateNodeRelationshipsForSymbols(
        [...indexedSymbols.values()],
        nodeMap,
      );
      ...
      this.logger.info(
        `Workspace graph built with ${nodeMap.size} nodes and ${edgeCount} edges.`,
      );
      if (this.db) {
        this.persistToDatabase();
      } else {
  ...
  private createNodeMap(
    indexedSymbols: Map<string, IndexedSymbol>,
  ...
  private async populateNodeRelationshipsForSymbols(
    ...
    for (const symbol of symbols) {
      ...
      if (this.isCallableSymbol(symbol.symbolKind)) {
        const outgoing = await this.indexer.resolveOutgoingCalls(
          symbol,
          this.symbolCache,
        );
        for (const targetId of outgoing.filter(
          (targetNodeId) =>
            targetNodes.has(targetNodeId) && targetNodeId !== symbol.id,
        )) {
      ...
      const implementations = await this.indexer.resolveImplementations(
        symbol,
        this.symbolCache,
      );
      for (const targetId of implementations.filter(
        (targetNodeId) =>
          targetNodes.has(targetNodeId) && targetNodeId !== symbol.id,
      )) {
        const targetNode = targetNodes.get(targetId);
      ...
      if (this.isVariableLikeSymbol(symbol.symbolKind)) {
        const variableReferences = await this.indexer.resolveVariableReferences(
          symbol,
          this.symbolCache,
        );
        const filteredReferences = this.filterReferenceBuckets(
          variableReferences,
          targetNodes,
          symbol.id,
        );
  ...
  private trimDanglingRelationships(): void {
    for (const node of this.cachedGraph.nodes.values()) {
      node.outgoingCalls = node.outgoingCalls.filter(
        (targetId) =>
          this.cachedGraph.nodes.has(targetId) && targetId !== node.id,
      );
      node.implementations = node.implementations.filter(
        (targetId) =>
          this.cachedGraph.nodes.has(targetId) && targetId !== node.id,
      );
      node.references.reads = node.references.reads.filter(
        (targetId) =>
          this.cachedGraph.nodes.has(targetId) && targetId !== node.id,
      );
      node.references.writes = node.references.writes.filter(
        (targetId) =>
          this.cachedGraph.nodes.has(targetId) && targetId !== node.id,
      );
    }
  ...
  private rebuildFileRelationshipIndex(
    nodeMap: Map<string, GraphNode> = this.cachedGraph.nodes,
  ): void {
  ...
  private linkRelationship(
    ...
    switch (edgeType) {
      ...
      case "writes":
        ...
        this.addUniqueValue(
          targetNode.incomingReferences.writes,
          sourceNode.id,
        );
        break;
      default:
  ...
  private removeRelationshipByDetails(
    sourceId: string,
    targetId: string,
    edgeType: GraphEdgeType,
  ): void {
    ...
    switch (record.edgeType) {
      ...
      case "implements":
        ...
        this.removeValueFromArray(
          targetNode.incomingImplementations,
          record.sourceId,
        );
        break;
      case "reads":
        ...
        this.removeValueFromArray(
          targetNode.incomingReferences.reads,
          record.sourceId,
        );
        break;
      case "writes":
        this.removeValueFromArray(
          sourceNode.references.writes,
          record.targetId,
        );
        this.removeValueFromArray(
          targetNode.incomingReferences.writes,
  ...
  private removeRelationshipRecordFromIndex(
    ...
    const filtered = records.filter(
      (entry) => !this.sameRelationshipRecord(entry, record),
    );
  ...
  private sameRelationshipRecord(
    left: FileRelationshipRecord,
    right: FileRelationshipRecord,
  ): boolean {
    return (
  ...
  private async rebuildSupplementaryFileRelationships(
    filesByRole: Record<WorkspaceFileRole, vscode.Uri[]>,
  ): Promise<void> {
    ...
    for (const [role, uris] of Object.entries(filesByRole) as Array<
      [WorkspaceFileRole, vscode.Uri[]]
    >) {
  ...
  private async extractSupplementaryFileRelationships(
    ...
    const addRelationship = (
      ...
      relationship: WorkspaceFileRelationshipType,
    ): void => {
    ...
    for (const specifier of this.extractImportSpecifiers(text)) {
      const targetFilePath = this.resolveWorkspaceTargetPath(
        sourceFilePath,
        specifier,
        knownFileLookup,
      );
    ...
    if (role === "test") {
      for (const targetFilePath of this.inferTestCoverageTargets(
        sourceFilePath,
        knownFileLookup,
      )) {
  ...
  private inferTestCoverageTargets(
    sourceFilePath: string,
    knownFileLookup: Map<string, string>,
  ): string[] {
    const normalizedSourceBase = this.stripTestSuffix(
      path.posix.basename(sourceFilePath),
    );
    ...
    for (const extension of extensions) {
      candidates.add(
        path.posix.normalize(
          path.posix.join(sourceDir, `${normalizedSourceBase}${extension}`),
        ),
      );
      candidates.add(
        path.posix.normalize(
          path.posix.join(sourceDir, normalizedSourceBase, `index${extension}`),
        ),
      );
    }
  ...
  private resolveWorkspaceTargetPath(
    ...
    for (const candidate of [...candidates]) {
      ...
      for (const expandedCandidate of this.expandRelationshipPathCandidates(
        candidate,
      )) {
        const expandedResolved = this.findKnownFilePath(
          expandedCandidate,
          knownFileLookup,
        );
        if (expandedResolved) {
  ...
  private stripTestSuffix(fileName: string): string {
    ...
    const withoutSuffix = withoutExtension.replace(
      /(?:\.|-|_)?(?:test|spec)$/i,
      "",
    );
  ...
  private collectKnownFileUriLookup(
    filesByRole?: Record<WorkspaceFileRole, vscode.Uri[]>,
  ): Map<string, string> {
  ...
  private createKnownFilePathLookup(
  ...
  private findKnownFilePath(
    ...
    for (const [key, filePath] of knownFileLookup) {
      if (
        key.endsWith(`/${normalizedCandidate.toLowerCase()}`) ||
  ...
  private async readWorkspaceFileText(
    uri: vscode.Uri,
  ...
  private async enqueuePersistence(): Promise<void> {
    ...
    this.persistQueue = this.persistQueue
      ...
      .catch((error) => {
        this.logger.warn(
          `Unable to persist VSContext graph cache. ${String(error)}`,
        );
      });
  ...
  private async persistSnapshot(): Promise<void> {
    ...
    const directoryUri = vscode.Uri.file(
      path.dirname(this.cacheFileUri.fsPath),
    );
    ...
    const snapshot: SerializedWorkspaceGraphSnapshot = {
      ...
      fileRoleSummary: this.cachedGraph.fileRoleSummary,
      fileRelationships: this.flattenSupplementaryFileRelationships(),
      nodes: [...this.cachedGraph.nodes.values()].map((node) =>
    ...
    try {
      db.transaction(() => {
        ...
        db.replaceAllFileRelationships(
          this.flattenSupplementaryFileRelationships(),
        );

        // Write symbol cache.
        for (const [id, sym] of this.symbolCache) {
          db.upsertSymbolCache(id, JSON.stringify(serializeIndexedSymbol(sym)));
        }
        ...
        if (this.cachedGraph.fileRoleSummary) {
          db.setMetadata(
            "fileRoleSummary",
            JSON.stringify(this.cachedGraph.fileRoleSummary),
          );
        }
      });
      ...
      this.logger.warn(
        `[VSContext] Failed to persist graph to database: ${error}`,
      );
    }
  }
  ...
  private parseSnapshot(
    value: unknown,
  ): SerializedWorkspaceGraphSnapshot | undefined {
    ...
    if (
      !candidate.fileModifiedTimes ||
      typeof candidate.fileModifiedTimes !== "object"
    ) {
      return undefined;
    }

    const safeWorkspaceUri =
      typeof candidate.workspaceFolderUri === "string"
        ? candidate.workspaceFolderUri
        : undefined;
    const safeSavedAtIso =
      typeof candidate.savedAtIso === "string" ? candidate.savedAtIso : "";
    if (!safeSavedAtIso) {
      return undefined;
    }

    const safeBuiltAtIso =
      typeof candidate.builtAtIso === "string"
        ? candidate.builtAtIso
        : undefined;
    const fileRoleSummary = this.parseFileRoleSummary(
      candidate.fileRoleSummary,
    );
  ...
  private deserializeWorkspaceFileRelationships(
    value: unknown,
  ): WorkspaceFileRelationship[] {
  ...
  private parseFileRoleSummary(
    value: unknown,
  ): WorkspaceFileRoleSummary | undefined {
    if (!value || typeof value !== "object") {
      return undefined;
    }
  ...
  private deserializeNode(node: SerializedGraphNode): GraphNode | undefined {
    ...
    const outgoingCalls = node.outgoingCalls.filter(
      (entry) => typeof entry === "string",
    );
    const implementations = node.implementations.filter(
      (entry) => typeof entry === "string",
    );
    const referenceReads = node.referenceReads.filter(
      (entry) => typeof entry === "string",
    );
    const referenceWrites = node.referenceWrites.filter(
      (entry) => typeof entry === "string",
    );
  ...
  private deserializeFileModifiedTimes(
  ...
  private trimDanglingRelationshipsFor(nodeMap: Map<string, GraphNode>): void {
    for (const node of nodeMap.values()) {
      node.outgoingCalls = node.outgoingCalls.filter(
        (targetId) => nodeMap.has(targetId) && targetId !== node.id,
      );
      node.implementations = node.implementations.filter(
        (targetId) => nodeMap.has(targetId) && targetId !== node.id,
      );
  ...
  private async refreshTrackedFileModifiedTimes(
  ...
  private async getFileModifiedTime(
    uri: vscode.Uri,
  ...
  private logIndexingSummary(indexResult: WorkspaceIndexResult): void {
    this.logger.info(
      `[VSContext] Indexed ${indexResult.scannedFileCount} files.`,
    );
    this.logger.info(
      `[VSContext] Indexed ${indexResult.indexedSymbolCount} symbols.`,
    );
    this.logger.info(
      `[VSContext] Skipped dependency directories: ${indexResult.skippedByExclusions} files.`,
    );
    this.logger.info(
  ...
  private filterReferenceBuckets(
    ...
    return {
      reads: references.reads.filter(
        (sourceId) => targetNodes.has(sourceId) && sourceId !== symbolId,
      ),
      writes: references.writes.filter(
        (sourceId) => targetNodes.has(sourceId) && sourceId !== symbolId,
      ),
    };
  }
  ...
  private logGraphNodeCreation(symbol: IndexedSymbol): void {
    ...
    const kindLabel =
      (vscode.SymbolKind as unknown as Record<number, string>)[
        symbol.symbolKind
      ] ?? symbol.symbolKind.toString();
    this.logger.info(
      `[VSContext][debug] Creating graph node: ${symbol.symbolName} (${kindLabel})`,
    );
  }
