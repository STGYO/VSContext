...
/**
 * Optional data-flow hints extracted from the AST alongside a symbol's name
 * and location.  All fields are optional so that language extractors can
 * provide partial information — partial hints are better than none.
 */
interface SymbolHints {
  readonly isAsync?: boolean; // true if async function/method
  readonly hasBody?: boolean; // true if has a body (not just a declaration)
  readonly parameterCount?: number; // number of formal parameters
  readonly variablesModified?: string[]; // variable names written inside this scope
  readonly variablesRead?: string[]; // variable names read from an outer scope
}
...
interface WorkerExtractedSymbol {
  ...
  // NEW: Data flow hints (all optional for backward compatibility)
...
/**
 * Result emitted by the worker thread after scanning a batch of files.
 * `symbolMap` values now carry the enhanced WorkerExtractedSymbol shape
 * including optional data-flow hints populated by each language extractor.
 */
...
const LANGUAGE_BY_EXTENSION: Record<
  string,
  { language: unknown; family: LanguageFamily } | undefined
> = {
...
const PY_FUNCTION_SCOPE_TYPES = new Set<string>([
  "function_definition",
  "async_function_definition",
  "lambda",
]);
const RUST_METHOD_SCOPE_TYPES = new Set<string>(["impl_item", "trait_item"]);
const C_TYPE_SCOPE_TYPES = new Set<string>([
  "struct_specifier",
  "class_specifier",
]);
...
function isValidIdentifier(value: string): boolean {
  ...
  for (let index = 1; index < value.length; index += 1) {
    ...
    const isTrailingMethodPunctuation =
      index === value.length - 1 && (code === 33 || code === 63);
...
function isUpperSnakeCase(value: string): boolean {
  ...
  for (let index = 0; index < value.length; index += 1) {
    const code = value.charCodeAt(index);
    const isValid =
      (code >= 65 && code <= 90) || (code >= 48 && code <= 57) || code === 95;
...
function getParserForExtension(
  extension: string,
): { parser: Parser; family: LanguageFamily } | undefined {
...
function walkNamed(
  ...
  while (stack.length > 0) {
    ...
    for (
      let childIndex = current.namedChildCount - 1;
      childIndex >= 0;
      childIndex -= 1
    ) {
...
function selfTestCollectDeclaratorNames(): void {
  ...
  const makeNode = (
    overrides: Partial<Parser.SyntaxNode>,
  ): Parser.SyntaxNode => {
    const base: AnyNode = {
      type: "root",
      childCount: 0,
  ...
  console.assert(
    namesMultiple.length === 2,
    "Expected two names for multiple declarators",
  );
  console.assert(
    namesMultiple[0] === id1 && namesMultiple[1] === id2,
    "Names should be collected in order",
  );
  ...
  console.assert(
    namesNested.length === 1,
    "Expected one name for nested declarator",
  );
  console.assert(
    namesNested[0] === nestedId,
    "Nested declarator name should be collected via findNameNode",
  );
}
 ...
 */
function hasChildToken(node: Parser.SyntaxNode, tokenType: string): boolean {
  for (let index = 0; index < node.childCount; index += 1) {
    const child = node.child(index);
...
function extractFunctionHints(
  ...
  const parameterCount =
    paramsNode !== null ? paramsNode.namedChildCount : undefined;

  let variablesModified: string[] | undefined;
  if (bodyNode !== null) {
...
function extractTypeScriptLikeSymbols(
  root: Parser.SyntaxNode,
  collector: SymbolCollector,
): void {
  walkNamed(root, (node) => {
    switch (node.type) {
      ...
      case "interface_declaration": {
        collector.addFromNode(
          node.childForFieldName("name") ?? findNameNode(node),
          "interface",
        );
        break;
      }
      ...
      case "namespace_declaration": {
        collector.addFromNode(
          node.childForFieldName("name") ?? findNameNode(node),
          node.type === "module_declaration" ? "module" : "namespace",
        );
        break;
      }
      ...
      case "abstract_method_signature": {
        collector.addFromNode(
          node.childForFieldName("name") ?? findNameNode(node),
          "method",
          extractFunctionHints(node, collector),
        );
        break;
      }
  ...
  walkNamed(root, (node) => {
    switch (node.type) {
      ...
      case "async_function_definition": {
        const kind: WorkerSymbolKind = hasAncestorType(
          node,
          PY_CLASS_SCOPE_TYPES,
        )
          ? "method"
          : "function";
        const pyBodyNode = node.childForFieldName("body");
        const pyHints: SymbolHints = {
      ...
      case "augmented_assignment": {
        ...
        if (target.type === "attribute") {
          ...
          const attributeNode =
            target.childForFieldName("attribute") ?? findNameNode(target);
  ...
  walkNamed(root, (node) => {
    switch (node.type) {
      ...
      case "function_item": {
        const kind: WorkerSymbolKind = hasAncestorType(
          node,
          RUST_METHOD_SCOPE_TYPES,
        )
          ? "method"
          : "function";
        const rsBodyNode = node.childForFieldName("body");
        const rsHints: SymbolHints = {
      ...
      case "let_declaration": {
        const names: Parser.SyntaxNode[] = [];
        collectIdentifierNodes(
          node.childForFieldName("pattern") ?? node,
          names,
        );
        for (const nameNode of names) {
          collector.addFromNode(nameNode, "variable");
    ...
    switch (node.type) {
      ...
      case "field_declaration": {
        ...
        for (let index = 0; index < node.namedChildCount; index += 1) {
          ...
          const nameNode =
            child.childForFieldName("name") ?? findNameNode(child);
          if (!nameNode) {
            continue;
          }
...
function findFunctionDeclaratorName(
...
function extractCStyleSymbols(
  ...
  walkNamed(root, (node) => {
    switch (node.type) {
      ...
      case "function_definition": {
        const declarator =
          node.childForFieldName("declarator") ?? findNameNode(node);
        ...
        const kind: WorkerSymbolKind = declaratorText.includes("::")
          ? "method"
          : "function";
...
function extractCSharpSymbols(
  ...
  walkNamed(root, (node) => {
    switch (node.type) {
      ...
      case "field_declaration": {
        const isConst =
          hasModifierToken(node, collector, "const") ||
          hasModifierToken(node, collector, "readonly");
        for (const nameNode of collectDeclaratorNames(
          node,
          "variable_declarator",
        )) {
          const name = collector.readText(nameNode);
      ...
      case "assignment_expression": {
        ...
        if (receiverText === "this" || receiverText === "base") {
          collector.addFromNode(
            member,
            hasAncestorType(node, CSHARP_TYPE_SCOPE_TYPES)
              ? "field"
              : "property",
          );
        }
  ...
  walkNamed(root, (node) => {
    switch (node.type) {
      ...
      case "function_declaration": {
        ...
        const inFunctionScope = hasAncestorType(
          node,
          KOTLIN_FUNCTION_SCOPE_TYPES,
        );
        collector.addFromNode(
          node.childForFieldName("name") ?? findNameNode(node),
          inTypeScope && !inFunctionScope ? "method" : "function",
        );
        break;
      }
      ...
      case "property_declaration": {
        ...
        const isTopLevelVal =
          /\bval\b/.test(declarationText) && !inFunctionScope && !inTypeScope;
        const name = collector.readText(nameNode);
...
async function run(): Promise<void> {
  const input = (workerData as WorkerBatchInput | undefined) ?? {
    filePaths: [],
  };
